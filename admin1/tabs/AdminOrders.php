...
class AdminOrders extends AdminTab
{
				...
				<div style="float:left;">
					<table style="width: 868px;" cellspacing="0" cellpadding="0" class="table" id="orderProducts">
						<tr>
							...
							<th style="width: 80px; text-align: center">'.$this->l('Unit Price').' <sup>*</sup></th>
							<th style="width: 20px; text-align: center">'.$this->l('Quantity').'</th>
							'.($has_been_paid ? '<th style="width: 20px; text-align: center">'.$this->l('Refunded').'</th>' : '').'
							'.($has_been_delivered ? '<th style="width: 20px; text-align: center">'.$this->l('Returned').'</th>' : '').'
							<th style="width: 30px; text-align: center">'.$this->l('Stock').'</th>
							<th style="width: 90px; text-align: center">'.$this->l('Total').' <sup>*</sup></th>
							<th style="width: 120px;"><img src="../img/admin/delete.gif" alt="'.$this->l('Products').'" /> '.(($has_been_delivered || $order->hasBeenShipped()) ? $this->l('Return') : ($has_been_paid ? $this->l('Refund') : $this->l('Cancel'))).'</th>
						</tr>';
					...
					echo '</table>
					<div style="float:left; width:370px; margin-top:15px;"><sup>*</sup> '.$this->l('According to this customer\'s group, prices are displayed:').' '.($order->getTaxCalculationMethod() == PS_TAX_EXC ? $this->l('tax excluded.') : $this->l('tax included.')).(!Configuration::get('PS_ORDER_RETURN') ? '<br /><br />'.$this->l('Merchandise returns are disabled') : '').'</div>';
					if (sizeof($discounts))
					{
	...
	public function display()
	{
		...
		else
		{
			...
			echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/boom.html');
			echo '<h2 class="space" style="text-align:right; margin-right:44px;">'.$this->l('Total:').' '.Tools::displayPrice($this->getTotal(), $currency).'</h2>';
			
			echo '<div class="path_bar">GIFTS</div>';
			echo '<ul id="submenu" class="withLeftBorder clearfix">';
			echo '<li><a href="index.php?tab=AdminOrders&token='.$this->token.'">Все</a></li>';
			echo '<li><a href="index.php?tab=AdminOrders&token='.$this->token.'&gifts=pending">На ожидании</a></li>';
			echo '<li><a href="index.php?tab=AdminOrders&token='.$this->token.'&gifts=shipping">В доставке</a></li>';
			echo '<li><a href="index.php?tab=AdminOrders&token='.$this->token.'&gifts=delivered">Доставлены</a></li>';
			echo '</ul>';
			echo '<table width="100%"><tbody><tr><td><table class="table" id="gifts_tab" cellpadding="0" cellspacing="0" width="100%"><thead><tr class="nodrag nodrop">';
			echo '<th>№</th>';
			echo '<th width="85">Статус</th>';
			echo '<th>Заказ, №</th>';
			echo '<th>Товар</th>';
			echo '<th width="200">Заказчик</th>';
			echo '<th>Стоимость</th>';
			if (@$_GET['gifts'] == 'pending') echo '<th width="130">Дата доставки</th>';
			else echo '<th width="130">Действия</th>';
			echo '</tr>';
			...
			if (@$_GET['giftact']) {
				switch($_GET['giftact']) {
					case 'order' :
						$url = 'https://boomstarter.ru/api/v1.1/partners/gifts/'.$_GET['uuid'].'/order';
					    $post = 'shop_uuid=27bb3bf8-f7a2-49e9-8445-9d062c7d3871&shop_token=f9e3218e-4eb1-4c03-8e40-fa4b41a0a4b2&order_id='.(10000000 + rand(1,100000));
						break;
					case 'schedule' :
						if (@$_GET['data']) {
							$url = 'https://boomstarter.ru/api/v1.1/partners/gifts/'.$_GET['uuid'].'/schedule';
						    $post = 'shop_uuid=27bb3bf8-f7a2-49e9-8445-9d062c7d3871&shop_token=f9e3218e-4eb1-4c03-8e40-fa4b41a0a4b2&delivery_date='.$_GET['data'];
						}
					    break;
					case 'delivery_state' :
						$url = 'https://boomstarter.apiary.io/api/v1.1/partners/gifts/'.$_GET['uuid'].'/delivery_state';
					    $post = 'shop_uuid=27bb3bf8-f7a2-49e9-8445-9d062c7d3871&shop_token=f9e3218e-4eb1-4c03-8e40-fa4b41a0a4b2&delivery_state=delivery';
						break;
				}
				if (@$url) {
					$ch = curl_init($url);
				...
				header('Location: /admin1/index.php?tab=AdminOrders&token='.$this->token);
				exit();
			}
			
			$status = array('none'=>'Не подтвержден', 'accepted'=>'На ожидании', 'shipping'=>'В доставке', 'delivered'=>'Доставлен');
			$gifts = json_decode(file_get_contents('https://boomstarter.ru/api/v1.1/partners/gifts'.(@$_GET['gifts'] ? '/'.strip_tags($_GET['gifts']) : '').'?shop_uuid=27bb3bf8-f7a2-49e9-8445-9d062c7d3871&shop_token=f9e3218e-4eb1-4c03-8e40-fa4b41a0a4b2'),1);
		   	
		   	foreach($gifts['gifts'] as $g) {
				echo '<tr class="nodrag nodrop" style="height: 35px;" id="tr-'.$g['uuid'].'">';
				echo '<td class="center">'.$g['product_id'].'</td>';
				echo '<td class="center">'.$status[$g['delivery_state']].($g['delivery_date'] ? '<br />'.$g['delivery_date'] : '').'</td>';
				echo '<td class="center">'.$g['order_id'].'</td>';
				echo '<td>'.$g['name'].'</td>';
				echo '<td>'.$g['owner']['first_name'].' '.$g['owner']['last_name'].'<br />'.$g['owner']['phone'].', '.$g['owner']['email'].'<br />'.$g['location']['country']['name'].', '.$g['location']['city']['name'].'</td>';
				echo '<td>'.number_format($g['pledged_cents'], 2, ',', ' ').'</td>';
				echo '<td>';
				if ($g['delivery_state'] == 'none') echo '<a href="index.php?tab=AdminOrders&token='.$this->token.'&giftact=order&uuid='.$g['uuid'].'" class="button">подтвердить</a>';
				elseif ($g['delivery_state'] == 'accepted') echo '<form action="/admin1/index.php" method="get"><input type="hidden" name="tab" value="AdminOrders"><input type="hidden" name="token" value="'.$this->token.'"><input type="hidden" name="giftact" value="schedule"><input type="hidden" name="uuid" value="'.$g['uuid'].'"><input name="data" type="text" value="" size="10"> <input type="submit" value="ok" class="button"></form>';
				elseif ($g['delivery_state'] == 'shipping') echo '<a href="javascript:;" onClick="delivered(\''.$g['uuid'].'\');" class="button">доставлен</a>';
				else echo '--';
				echo '</td></tr>';
			}
